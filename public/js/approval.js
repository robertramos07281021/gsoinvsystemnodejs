...
Array.from(approvalButton).forEach(async(element,index)=> {
  ...
  const approvalData = await axios(`http://localhost:8000/user/requests/${approvalId}`);
...
Array.from(forReceivingDetailsButton).forEach(async(element,index) => {
  ...
  const forReceivingData = await axios(`http://localhost:8000/user/requests/${forReceivingId}`);
...
Array.from(completeReceivedButton).forEach(async(element,index) => {
  ...
  const receivedData = await axios(`http://localhost:8000/user/requests/${completeReceivedId}`);
