...
for(i = 0; i < requestButton.length; i++) {
  ...
  requestButton[i].onclick = async function () {
    const deployData = await axios(`http://localhost:8000/items/manage/deploy/${itemId}`)
...
for (let l = 0; l < ReceiveBtn.length; l++) {
  ReceiveBtn[l].onclick = async function () {
    ...
    const res = await axios(`http://localhost:8000/user/requests/${dataAttribute}`)
...
for(let k = 0; k < completeReceiveBtn.length; k++) {
  completeReceiveBtn[k].onclick = async function () {
    ...
    const completeReceiveRes = await axios(`http://localhost:8000/user/requests/${receivedDataAttribute}`)
...
for(let h = 0; h < editMyRequestBtn.length; h++) {
  ...
  editMyRequestBtn[h].onclick = async function () {
    const requestEditData = await axios(`http://localhost:8000/user/requests/${requestEditDataAttribute}`);
...
for(let t = 0; t < deleteThisRequestBtn.length; t++) {
  ...
  deleteThisRequestBtn[t].onclick = async function() {
    ...
    const requestEditData = await axios(`http://localhost:8000/user/requests/${requestDeleteDataAttribute}`);
