...
myProfileEditButton.onclick = async function () {
  ...
  const userData = await axios(`https://gsoinvsystemnodejs.onrender.com/user/${userId}`)
  console.log(userId)
  ...
  for(let k = 0 ; k < 6; k++ ) {
    ...
    if(k == 4) {
      const department = await axios(`https://gsoinvsystemnodejs.onrender.com/department`)
