...
Array.from(deleteDeptButton).forEach((element, index) => {
  ...
  element.onclick =async function () {
    ...
    const getDept = await axios(`https://gsoinvsystemnodejs.onrender.com/department/${deptId}`);
...
Array.from(editDeptButton).forEach(async (element,index) => {
  const deptId = deptData[index].getAttribute('data-dept-id');
  const findDept = await axios(`https://gsoinvsystemnodejs.onrender.com/department/${deptId}`)
...
Array.from(historyDetailsButton).forEach(async (element, index) =>{
  ...
  element.onclick = async function () {
    ...
    const history = await axios(`https://gsoinvsystemnodejs.onrender.com/history/${historyId}`)
