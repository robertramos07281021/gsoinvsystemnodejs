...
exports.usersPage = catchAsync(async (req, res) => {
  ...
  if (Boolean(usersDeactiveQueryPages)) {
    const paginateDeactiveUser = paginate(
      usersDeactive,
      Number(usersDeactiveQueryPages),
      8
    );
  ...
  res.render("gsopages/users", {
    ...
    usersQueryPages,
    usersPaginate,
    usersDeactiveQueryPages,
    usersDeactivePaginate,
  });
});
...
exports.reportsPage = catchAsync(async (req, res) => {
  ...
  const requests = await Request.find({ receive: { $eq: true } }).populate(
    "user"
  );
  ...
  res.render("gsopages/report", {
    units,
    deployeds,
    deptWithDeploys,
    requests,
    completeTransactionDepts,
  });
});
...
exports.printScreenDeployedUnits = catchAsync(async (req, res) => {
  ...
  if (!Boolean(req.query.q)) {
    ...
    const dept = await Dept.findOne({
      department: { $eq: req.query.q.toUpperCase() },
    });
    if (!dept) {
    ...
    const deployed = await Deploy.find({ _id: { $in: dept.deployed } })
      .populate("department")
      .populate("item");
    Array.from(deployed).forEach((element) => {
      deployedData.push(element);
...
exports.printScreenCompleteTransacion = catchAsync(async (req, res) => {
  const requestData = [];
  const request = await Request.find({ receive: { $eq: true } }).populate(
    "user"
  );
  ...
  if (Boolean(req.query.completeDept) && Boolean(req.query.dateQuery)) {
    Array.from(request).forEach((element) => {
      ...
      if (
        element.department.department == req.query.completeDept.toUpperCase() &&
...
exports.departmentPage = catchAsync(async (req, res) => {
  ...
  if (Boolean(req.query.selectedDept)) {
    if (departmentsId.includes(querySelector)) {
      const deployeds = await Deploy.find({
        department: { $eq: req.query.selectedDept },
      }).populate("item");
      ...
      const users = await User.find({
        department: { $eq: departmentName.department },
      });
    ...
    const deployeds = await Deploy.find({
      department: { $eq: departments[0].id },
    }).populate("item");
    for (let b = 0; b < deployeds.length; b++) {
      deployedUnits.push(deployeds[b]);
    }
    const users = await User.find({
      department: { $eq: departments[0].department },
    });
    for (let x = 0; x < users.length; x++) {
      usersOnDept.push(users[x]);
    }
  }
  res.render("gsopages/departments", {
    departments,
    querySelector,
    deployedUnits,
    usersOnDept,
...
exports.saveDepartment = catchAsync(async (req, res) => {
  const findDept = await Dept.find({
    department: { $eq: req.body.departmentHidden.toUpperCase() },
  });
  if (findDept.length > 0) {
    ...
    req.flash(
      "success",
      `You successfuly added ${req.body.departmentHidden.toUpperCase()}`
    );
...
exports.deleteDepartment = catchAsync(async (req, res) => {
  const { id } = req.params;
  const deleteDepartment = await Dept.findByIdAndDelete(id).populate(
    "deployed"
  );
  if (!deleteDepartment) {
  ...
  req.flash(
    "success",
    `You successfully deleted ${deleteDepartment.department}`
  );
  res.redirect("/departments/manage");
});
...
exports.editDepartment = catchAsync(async (req, res) => {
  const { id } = req.params;
  const ifDeptIsExist = await Dept.find({
    department: req.body.newNameOfDept.toUpperCase(),
  });
  if (ifDeptIsExist.length < 1) {
    const editDept = await Dept.findByIdAndUpdate(id, {
      department: req.body.newNameOfDept,
    });
    const history = new HistoryDept({
      action: "update",
      newname: req.body.newNameOfDept.toUpperCase(),
      user: req.user._id,
    });
    ...
    req.flash(
      ...
      `Successfully update ${
        dept.department
      } to ${req.body.newNameOfDept.toUpperCase()}`
    );
    res.redirect("/departments/manage");
  } else {
...
exports.mergeDeptartment = catchAsync(async (req, res) => {
  try {
    const dept = await Dept.find({
      department: { $in: req.body.mergeDeptChecked },
    });
    ...
    Array.from(dept).forEach((element) => {
      Array.from(element.deployed).forEach(async (deployed) => {
        ...
        const updateUnit = await Unit.findOneAndUpdate(
          { item: { $eq: deployUnit.item } },
          { $inc: { qty: Number(deployUnit.qty) } }
        );
    ...
    Array.from(user).forEach(async (user) => {
      await User.findByIdAndUpdate(user._id, {
        department: req.body.mergeDeptName,
      });
    });
    const deleted = await Dept.deleteMany({
      department: { $in: req.body.mergeDeptChecked },
    });

    const history = new HistoryDept({
      action: "merge",
      newname: req.body.mergeDeptName,
      user: req.user._id,
    });
...
exports.editUnit = catchAsync(async (req, res) => {
  ...
  const itemUpdate = await Item.findByIdAndUpdate(findUnit.item.id, {
    name: req.body.name,
    code: req.body.code,
    description: req.body.description,
  })
...
exports.deployUnit = catchAsync(async (req, res) => {
  ...
  const findDeploy = await Deploy.findOne({
    $and: [{ item: { $eq: unit.item.id } }, { department: { $eq: dept } }],
  });
  if (findDeploy) {
    const updateDeploy = await Deploy.findByIdAndUpdate(findDeploy, {
      $inc: { qty: Number(req.body.qty) },
    });
    const updateUnit = await Unit.findByIdAndUpdate(req.body.unitId, {
      $inc: { qty: -Number(req.body.qty) },
    });
    req.flash(
      "success",
      `Successfully deploy ${unit.item.name} to ${dept.department}`
    );
    res.redirect("/items/manage");
  } else {
    ...
    if (unit.qty < Number(req.body.qty)) {
      req.flash(
        "error",
        `You have ${unit.qty} total ${unit.qty > 1 ? "units" : "unit"} in ${
          unit.item.name
        }!`
      );
      res.redirect("/items/manage");
...
exports.deleteDeployUnit = catchAsync(async (req, res) => {
  const { id } = req.params;
  const deletedDeploy = await Deploy.findByIdAndDelete(id).populate(
    "department"
  );
  const updateDepartment = await Dept.findByIdAndUpdate(
    deletedDeploy.department.id,
    { $pull: { deployed: id } }
  );
...
exports.updateDeployUnit = catchAsync(async (req, res) => {
  const { id } = req.params;
  const updateDeploy = await Deploy.findByIdAndUpdate(id, {
    qty: Number(req.body.hiddenQty),
  }).populate("item");
  if (!updateDeploy) {
  ...
  req.flash(
    "updateEvent",
    `You successfully update ${updateDeploy.item.name} !`
  );
  res.redirect("/items/manage/deploy");
});
...
exports.deployApproval = catchAsync(async (req, res) => {
  const forApprovals = await Request.find({ approve: { $eq: false } })
    .populate({ path: "deployed", populate: { path: "item" } })
    .populate({ path: "deployed", populate: { path: "department" } })
    .populate("user");
  const forReceives = await Request.find({
    $and: [{ approve: { $eq: true } }, { receive: { $eq: false } }],
  })
    .populate({ path: "deployed", populate: { path: "item" } })
    .populate({ path: "deployed", populate: { path: "department" } })
    .populate("user");
  const completeReceived = await Request.find({
    receive: { $eq: true },
  }).populate("user");
  res.render("gsopages/approval", {
    forApprovals,
    forReceives,
    completeReceived,
  });
...
exports.deployApprove = catchAsync(async (req, res) => {
  ...
  const updateRequestApprove = await Request.findByIdAndUpdate(id, {
    approve: true,
    stamp: addStamp(8),
  ...
  const updateDeployed = await Deploy.findByIdAndUpdate(
    updateRequestApprove.deployed._id,
    { $inc: { qty: -Number(updateRequestApprove.qty) } }
  );
...
exports.deleteRequest = catchAsync(async (req, res) => {
  ...
  req.flash(
    "success",
    `You successfully delete request w/ tracking no ${deleteRequest.tracking}.`
  );
  res.redirect("/approval");
});
...
exports.userDashBoard = catchAsync(async (req, res) => {
  ...
  const currentUserDept = await Dept.find({
    department: { $eq: req.user.department },
  }).populate("deployed");
  const deployedUnitsOnYourDept = await Deploy.find({
    department: { $eq: currentUserDept[0] },
  }).populate("item");
  const availableUnits = await Unit.find().populate("item");
  const otherDeptUnits = await Deploy.find({
    department: { $ne: currentUserDept[0] },
  })
    .populate("item")
    .populate("department");
  res.render("gsousers/userdashboard", {
    deployedUnitsOnYourDept,
    availableUnits,
    otherDeptUnits,
    deptsWithDeploy,
  });
...
exports.userMyRequest = catchAsync(async (req, res) => {
  const depts = await Dept.find({ department: { $eq: req.user.department } });
  const deploy = await Deploy.find({ department: { $eq: depts[0] } }).populate(
    "item"
  );
  const request = await Request.find({
    $and: [{ user: { $eq: req.user._id } }, { approve: { $eq: false } }],
  }).populate("deployed");
  ...
  const receivedCount = await Request.countDocuments({
    $and: [{ user: { $eq: req.user._id } }, { receive: { $eq: true } }],
  });
  const receivedRequests = await Request.find({
    $and: [{ user: { $eq: req.user._id } }, { receive: { $eq: true } }],
  });
  res.render("gsousers/myrequest", {
    deploy,
    request,
    approvedRequests,
    requestCount,
...
exports.editMyRequest = catchAsync(async (req, res) => {
  ...
  if (
    Number(req.body.requestQty) > 0 ||
    Number(req.body.requestQty) < request.qty
  ) {
    const editMyRequestData = await Request.findByIdAndUpdate(id, {
      qty: Number(req.body.requestQty),
    });
    req.flash(
      "updateEvent",
      `You successfully update request, with tracking no ${editMyRequestData.tracking}`
    );
  } else {
...
exports.userAddRequest = catchAsync(async (req, res) => {
  const { id } = req.params;
  const deployUnit = await Deploy.findById(id)
    .populate("item")
    .populate("department");
  const department = await Dept.findById(deployUnit.department.id);
  const item = await Item.findById(deployUnit.item.id);
  if (
    Number(req.body.formRequestQty) > deployUnit.qty &&
    Number(req.body.formRequestQty) <= 0
  ) {
    ...
    const newRequest = new Request({
      ...
      qty: Number(req.body.formRequestQty),
      tracking: addTracking(5),
    });
    ...
    req.flash(
      "addEvent",
      `You successfully add ${Number(req.body.formRequestQty)} ${
        Number(req.body.formRequestQty) > 1 ? "requests" : "request"
      }, Wait for approval`
    );
    res.redirect("/user/myrequest");
  }
...
exports.userReceive = catchAsync(async (req, res) => {
  ...
  if (req.body.stampNumber === request.stamp) {
    const updateReceive = await Request.findByIdAndUpdate(id, {
      receive: true,
      receivedate: new Date(),
    });
    req.flash(
      "requestReceivedSuccess",
      `Item received with tracking no. ${updateReceive.tracking}`
    );
  } else {
...
exports.deleteMyRequest = catchAsync(async (req, res) => {
  const { id } = req.params;
  const deleteRequestInUser = await User.findByIdAndUpdate(req.user._id, {
    $pull: { request: id },
  });
  const deleteRequest = await Request.findByIdAndDelete(id);
  req.flash(
    "deleteEvent",
    `Request with tracking no. ${deleteRequest.tracking} has been deleted`
  );
  res.redirect("/user/myrequest");
});
...
exports.userReports = catchAsync(async (req, res) => {
  const requests = await Request.find({
    ...
    .populate({ path: "deployed", populate: "item" })
    .populate("user");
...
exports.printPreview = catchAsync(async (req, res) => {
  ...
  if (Boolean(query) == true) {
    for (let b = 0; b < request.length; b++) {
      if (
        query.includes(month[new Date(request[b].receivedate).getMonth()]) &&
        query.includes(new Date(request[b].receivedate).getDate()) &&
        query.includes(new Date(request[b].receivedate).getFullYear())
      ) {
...
exports.deployUnitApi = catchAsync(async (req, res) => {
  const { id } = req.params;
  const findDeployUnit = await Deploy.findById(id)
    .populate("item")
    .populate("department");
